...
├── memory/
│   └── history-manager.ts  # 双层历史管理（HISTORY.md 热缓冲 → SQLite 冷归档 → USER.md 提取）
├── middleware/
...
├── tools/
│   ├── basetool.ts         # BaseTool 抽象类 + ToolRegistry 自动发现 + 生命周期钩子
...
- **模糊搜索 + Tab 补全**：输入 `/` 触发命令模式，显示匹配建议面板，Tab 循环选中 / 唯一匹配直接补全
- **内置命令**：
  | 命令 | 别名 | 功能 |
  |------|------|------|
...
5. **记录** → 自动追加对话到 HISTORY.md → 超阈值时触发冷归档管道 → LLM 批量浓缩 → 提取用户特征到 USER.md
