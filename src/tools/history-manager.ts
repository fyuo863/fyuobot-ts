...
export class HistoryManager {
    ...
    async #callLLMCondense(prompt: string, toKeep: string): Promise<void> {
        try {
            ...
            const entries = this.#parseBatchResult(text);

            if (entries.length > 0) {
                this.#insertCondensed(entries);
                console.log(`  [历史] 浓缩完成：${entries.length} 条记录存入 SQLite`);
            }
    ...
    #parseBatchResult(text: string): Array<{ topic: string; summary: string }> {
        // 去 markdown 代码块
        let cleaned = text;
        ...
        try {
            ...
            if (Array.isArray(data)) {
                return data.filter(
                    (e): e is { topic: string; summary: string } =>
                        typeof e === "object" && e !== null && typeof e.summary === "string" && e.summary.length > 0,
                );
            }
        } catch {
        ...
        return this.#parseBatchFallback(cleaned);
    }
